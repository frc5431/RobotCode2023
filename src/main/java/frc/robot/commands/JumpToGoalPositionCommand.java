...
public class JumpToGoalPositionCommand extends CommandBase {
    ...
    public void execute() {
        ...
        if((flags & USE_INCHES) == USE_INCHES) {
            updatedPosition = new Translation2d(Units.inchesToMeters(updatedPosition.getX()),  Units.inchesToMeters(updatedPosition.getY()));
        }
    ...
    public boolean isFinished() {
        ...
        if((flags & USE_INCHES) == USE_INCHES) {
            dif = new Translation2d(Units.inchesToMeters(dif.getX()),  Units.inchesToMeters(dif.getY()));
        }
